...
const RockAutoUI = (() => {
  ...
  function renderCategories(container, data) {
    container.innerHTML = '';
    if (!data.categories?.length) { showEmpty(container, `No part categories found for this vehicle.`); return; }
  ...
  function renderParts(container, data) {
    ...
    container.appendChild(el('h3', { textContent: data.category ? `Parts — ${data.category} (${data.count})` : `Parts (${data.count})` }));
